...
public class EntryListTurtleWriter extends TurtleRepresentationWriter<EntryListView> {
    ...
    @Inject
    public EntryListTurtleWriter(javax.inject.Provider<EverythingAboutARegister> aboutARegisterProvider, RegisterResolver registerResolver) {
    ...
    protected Model rdfModelFor(EntryListView view) {
        ...
        for (Entry entry : view.getEntries()) {
            model.add(new EntryTurtleWriter(aboutARegisterProvider, registerResolver).rdfModelFor(entry));
        }
        return model;
    }
}
