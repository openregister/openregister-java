...
public abstract class RecordQueryDAO {
    ...
    @SqlQuery("select entry_number, timestamp, e.sha256hex as sha256hex, item_key, content from entry e, item i where e.sha256hex=i.sha256hex and e.entry_number = (select entry_number from current_keys where current_keys.key=:key)")
    @SingleValueResult(Record.class)
    @RegisterMapper(RecordMapper.class)
    public abstract Optional<Record> findByPrimaryKey(@Bind("key") String key);

    @SqlQuery("select entry.entry_number, timestamp, entry.sha256hex as sha256hex, entry.item_key, content from item, entry, current_keys where current_keys.entry_number = entry.entry_number and item.sha256hex=entry.sha256hex order by entry.entry_number desc limit :limit offset :offset")
    @RegisterMapper(RecordMapper.class)
    public abstract List<Record> getRecords(@Bind("limit") long limit, @Bind("offset") long offset);

    @SqlQuery("select entry_number, timestamp, sha256hex, item_key from entry where sha256hex in (select sha256hex from item where (content @> :contentPGobject)) order by entry_number asc")
    @RegisterMapper(EntryMapper.class)
    public abstract Collection<Entry> __findAllEntriesOfRecordBy(@Bind("contentPGobject") PGobject content);

    @SqlQuery("select entry.entry_number, timestamp, entry.sha256hex as sha256hex, entry.item_key, content from item, entry, current_keys where current_keys.entry_number = entry.entry_number and item.content @> :contentPGobject and item.sha256hex=entry.sha256hex limit 100")
    @RegisterMapper(RecordMapper.class)
